...
export default function PurchasePage() {
  ...
  const loadRecords = async () => {
    ...
    const res = await fetch(`/api/admin/purchase${qs ? `?${qs}` : ""}`);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("确定删除该进货记录？")) return;
    await fetch(`/api/admin/purchase/${id}`, { method: "DELETE" });
    loadRecords();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">进货台账</h1>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h2 className="font-semibold text-gray-700 mb-4">新增进货记录</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-4 py-3 text-left font-medium text-gray-600">品种名称</th>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">总成本(¥)</th>
              ...
              {records.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium text-gray-800">
                    {r.varietyName || `品种#${r.varietyId}`}
                  </td>
                  <td className="px-4 py-3 text-gray-600">{r.quantity}</td>
                  <td className="px-4 py-3 text-gray-600">¥{r.unitPrice.toFixed(2)}</td>
                  <td className="px-4 py-3 text-gray-800 font-medium">¥{r.totalCost.toFixed(2)}</td>
